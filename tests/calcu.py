...
class TestRegistrationForm:
    def test_registration(self, browser_context_args):
        ...
        logging.info("Divison of numbers")
        page.click("span[dcg-command='6']")
        page.click("span[dcg-command='/']")
        denominators = ["span[dcg-command='2']", "span[dcg-command='0']"]
        for denom in denominators:
            if denom == "span[dcg-command='0']":
                logging.warning("Division by zero is impossible.")
                continue
            else:
                page.click(denom)
